...
| `raw-output.txt` | Full annotated transcript — every command + its output, captured on the 5090. |
...
racecheck is scoped to the shared-memory kernels (`--kernel-name kns=Fused`):
`FusedOrig`/`FusedNew` are the only kernels declaring `__shared__`, and
instrumenting the full 2M-element workload overruns racecheck's tracker.
